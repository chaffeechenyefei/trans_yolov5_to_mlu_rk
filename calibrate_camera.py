...
def calibrate_camera():
    ...
    print(f'✓ 标定结果已保存: {opt.output} / Calibration saved to: {opt.output}')
    cv2.destroyAllWindows()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--camera_name', type=str, required=True,
                        help='相机名称, 如 camera_A / Camera name, e.g. camera_A')
    parser.add_argument('--camera_image', type=str, required=True,
                        help='Camera 截图路径 / Path to camera frame image')
    parser.add_argument('--plane_image', type=str, required=True,
                        help='2D 平面图路径 / Path to 2D floor plan image')
    parser.add_argument('--output', type=str, default='data/calibration/homography.json',
                        help='标定结果输出 JSON 路径 / Output JSON path for calibration data')
