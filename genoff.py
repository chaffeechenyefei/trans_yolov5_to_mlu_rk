...
def genoff_yolov5(model, mname, batch_size, core_number, in_height, in_width,
    ...
    from api_for_mlu import load_yolov5x6_model, load_yolo_model
    ...
    net = load_yolov5x6_model(detect_sz=IMG_SIZE, stride= model_context[model]['stride'],
                              cfg=model_context[model]['cfg'], weights=None, preTrained=loading)
    print('==end==')
    net = net.eval().float()
    net.model[-1].export = True  # set Detect() layer grid export
    net.model[-1].export_mode = 10 #special for mlu
