...
detect_sz = [w,h]
"""
def load_yolov5x6_model(detect_sz=[1024,1024], stride=[8., 16., 32., 64] ,cfg = './models/hub/yolov5x6.yaml',weights = './weights/yolov5x6.pth',preTrained=False, yolov5face=False):
    # weights = './weights/yolov5x6{}'.format('.pth')
    # cfg = './models/hub/yolov5x6.yaml'
    if yolov5face:
        from models.yolo_face import Model,Model_yolov5x6
    else:
        from models.yolo import Model,Model_yolov5x6
    yolo_net = Model_yolov5x6(cfg=cfg, input_sz=detect_sz ,stride=stride)
    # stride = int(yolo_net.stride.max())  # model stride
    # img_size = check_img_size(detect_sz, s=stride)  # check img_size
