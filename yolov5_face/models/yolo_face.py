...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=()):  # detection layer
        ...
        # self.anchor_grid = nn.Parameter(torch.tensor(anchors).float().reshape(self.nl, 1, -1, 1, 1, 2))
        # self.stride_ = nn.Parameter(torch.tensor([8., 16., 32., 64]))
        self.stride_ = []
        self.register_buffer('anchors', a)  # shape(nl,na,2)
        self.register_buffer('anchor_grid', a.clone().view(self.nl, 1, -1, 1, 1, 2))  # shape(nl,1,na,1,1,2)
        #special for yolov5x6
        # nxy = [128,64,32,16] #1024/8, 1024/16 , 1024/32 , 1024/64
        self.grid0 = None
        self.grid1 = None
        self.grid2 = None
        self.grid3 = None
        # self.grid0 = nn.Parameter(self._make_grid(128,128))
        # self.grid1 = nn.Parameter(self._make_grid(64,64))
        # self.grid2 = nn.Parameter(self._make_grid(32, 32))
        # self.grid3 = nn.Parameter(self._make_grid(16, 16))
    ...
    def set_single_output(self, singleOutput:bool):
    ...
    def forward(self, x):
        ...
        # if x[0].device.type == 'mlu' or x[0].device.type == 'cpu':
        ...
        #         xy = y[..., 0:2].sigmoid()
        #         wh = y[..., 2:4].sigmoid()
        #         obj_conf = y[..., 4:5].sigmoid()
        #         cls_conf  = y[..., 15:].sigmoid()
        #         xy = (xy * 2. - 0.5 + self.get_gridN(i)) * self.stride_[i]
        #         wh = (wh * 2) ** 2 * self.anchor_grid[i]
        #         land1 = y[..., 5:7]   *   self.anchor_grid[i] + self.get_gridN(i) * self.stride_[i]
        #         land2 = y[..., 7:9]   *   self.anchor_grid[i] + self.get_gridN(i) * self.stride_[i]
        #         land3 = y[..., 9:11]  *   self.anchor_grid[i] + self.get_gridN(i) * self.stride_[i]
        #         land4 = y[..., 11:13] *   self.anchor_grid[i] + self.get_gridN(i) * self.stride_[i]
        #         land5 = y[..., 13:15] *   self.anchor_grid[i] + self.get_gridN(i) * self.stride_[i]
        #         y = torch.cat([xy, wh, obj_conf, land1, land2, land3, land4, land5, cls_conf], dim=4)
        #
        #         output.append(y.reshape(bs, -1, self.no))
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None, singleOutput=False):  # model, input channels, number of classes
        ...
        m = self.model[-1]  # Detect()
        if isinstance(m, Detect):
            m.set_single_output(singleOutput=singleOutput)
            sh = 256  # 2x min stride
            sw = 512
            m.stride = torch.tensor([sw / x.shape[-2] for x in self.forward(torch.zeros(1, ch, sh, sw))])  # forward
            # print('m.stride',m.stride) #[8., 16., 32., 64]
