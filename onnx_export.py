...
def export_pt_to_onnx(weights_path: str, output_path: str,
    ...
    # NOTE: PyTorch >= 2.6 默认走 dynamo-based exporter (最低 opset 18).
    # YOLOv5 用的 Upsample 会生成 Resize 算子, dynamo 跑完 opset 18 后会
    # 自动调 onnx.version_converter 降到目标 opset, 但 Resize 在 18->11 路径上
    # 没有 adapter (报错: "No Adapter To Version $17 for Resize"), 模型被
    # "偷偷"以 opset 18 存盘. 强制 dynamo=False 走 legacy TorchScript exporter,
    # 它原生支持 opset 11/12, 不触发版本转换, 输出 opset 与参数一致.
    # PyTorch < 2.6 没有 dynamo 参数, 用 inspect 自动兼容.
    try:
        torch.onnx.export(
    ...
    # 读回元信息二次确认 opset 真的是参数指定的值 / Reload to confirm opset matches
    ...
    # ---- 导出后回载验证 / Post-export verification: load ONNX back and run inference ----
    print('-' * 40)
    ...
    try:
        ...
        # 使用与导出时相同的 dummy 输入 / Use same dummy input as export
        ...
        # 对比 PT 与 ONNX 输出的形状一致性 / Compare PT vs ONNX output shapes
