...
debug_dir = '/mnt/projects/ObjectDetection/dataset/SafetyHat/orig_cls'
class Classifier():
    def __init__(self, num_classes=3,
                 weights_fp='/mnt/projects/PracticeProjects/HatClassifier/weights/bestmodel34.pth'):
        ...
        self.resnet.eval()
        self.id2cls = {0: 'hat', 1: 'head', 2: 'others'}
        self.head_id = 0
    ...
    def inference(self, img, label):
        ...
        bbox = self.label2bbox(img, label)
        img_bbox = img[bbox[2]: bbox[4], bbox[1]: bbox[3], :]
        cv2.imwrite(os.path.join(debug_dir, f'{self.head_id}_bbox.jpg'), img_bbox)
        img_head = self.detect_head(img_bbox)
        cv2.imwrite(os.path.join(debug_dir, f'{self.head_id}_{cls}.jpg'), img_head)
        self.head_id += 1
        return cls
